...
# Constants for text
WARNING = R + "Command not recognized!" + W
...
smtpbrute_menu =  LR + """
    ...
    start bruteforce   | Start the attack once everything is set"
    ---
    exit bruteforce    | Exit bruteforce\n       """ + W
...
http_menu = LR + """
    ...
    driftnet            | Dumps JPEG/PNG traffic on network
    ---
    list               | Show this list again
    exit               | Go back to the main menu\n   """ + W
...
misc_menu = LR + """
    [Miscellanous Module - Avaiable Attack Vectors:]""" + G + """

    ---
    arpspoof            | ARP Spoof/Poison attack to capture packets on the network
    pingofdeath         | Ping-of-death attack
    ip6dos              | DoS attack on IPv6 Addresses
    pdfbrute            | Bruteforce an encrypted PDF file
    ---
    list               | Show this list again
    exit               | Go back to the main menu\n   """ + W
...
arpspoof_menu = LR + """
    ...
    ---
    exit arpspoof      | Exit ARPspoof\n     """ + W

pingofdeath_menu = LR + """
    [Ping of Death Attack Vector:]""" + G + """

    target <ip>        | Set the target's IP address
    requests <num>     | Number of ping requests to send
    threads <num>      | Number of threads to implement
    ---
    exit pingofdeath   | Exit pingofdeath\n  """ + W
